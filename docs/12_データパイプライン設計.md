...
| 処理内容 | 1) **停留所の重複統合**: 複数フィードにまたがる同一地点の停留所（例: 駅前のバス停が複数事業者のGTFSに別IDで登録されているケース）を、緯度経度の近接度と名称類似度で名寄せし、探索上は単一ノードとして扱う。2) **徒歩transfer事前計算**: 直線距離ベースで上限800mまでの停留所ペアを走査し、徒歩transferエッジとして事前計算する（Haversine距離を用いる。`10_GTFS-Flex実装仕様.md` 3.2節の`HaversineDurationEstimator`と同種の距離計算関数を共有する）。**ただし全ペアは保持しない。1停留所あたり距離昇順で上限K=8件のみを採用する**（2026-08-28改訂。実データ152,821停留所での計測で800m以内のペアが1,713,091件・概算91.5MBに達し、4章のサイズ予算3MBと両立しないことが判明したため。熊本4.5MB・高知4.0MB・長野3.5MB・三重3.2MBがtransferだけで予算超過していた。上限K=8は1)の名寄せ（20m閾値）と併用して353,073ペア・18.9MB・都道府県平均411KBとなり、接続される事業者ペアは上限なし時の1,371組に対し1,318組=96%を維持する。**半径縮小ではなく上限Kを採る理由**: 800m以内に他事業者停留所が無い停留所が87,070件(57%)あり、他事業者接続を持つ停留所のうち最近傍が300m超が31.2%を占めるため、半径を300mに縮めると約31%の停留所が事業者間乗換を失う。上限K方式は疎な地域でも最も近いK件を必ず保持するため、交通空白地域を対象とする本プロダクトでは損失が構造的に小さい。根拠と全測定値は`20_企画確定.md` 1.5節を正とする。都道府県分割後に県別実サイズを測り、3MB超過県のみKを下げる適応運用とする）。なお1)の名寄せは到達可能性を増やす処理ではない（近接停留所は名寄せしなくても本徒歩transferで接続される）。名寄せの効果はノード数削減（実測31.8%）・同一地点が「乗換」と表示されるのを防ぐUX・本節のtransfer件数削減である（`17_課題管理.md` C-25）。3) **カレンダー展開**: `calendar.txt`/`calendar_dates.txt`の曜日パターン・例外日を、実行日（ビルド実行日）から向こう35日分の具体的な運行日リストに展開し、シャードには展開済みの日付リストを格納する（実行時の`packages/router`が曜日ロジックを都度計算しなくて済むようにする事前計算）。4) Flexフィードについては`10_GTFS-Flex実装仕様.md` 3.1節のFlexLeg生成方針に従い、location_groupごとの仮想レッグの元となるデータ（停留所集合・時間窓・予約ルール参照）をグラフに組み込む（実際の120×119通りの全展開は行わず、4.2節で述べる遅延評価可能な表現でシャードに格納する） |
...
> **2026-08-28注記**: 徒歩transferを3.4節の原設計（800m以内の全ペア）で生成すると、この3MB予算を複数県で超過する（実測: 全国91.5MB、熊本4.5MB・高知4.0MB・長野3.5MB・三重3.2MB）。3.4節を「20m名寄せ＋1停留所あたり上限K=8件」へ改訂し、都道府県平均411KBに収めた。測定値は `20_企画確定.md` 1.5節を正とする。
...
interface Shard {
  ...
  bookingRules: CompressedBookingRules | null; // 予約ルールの正本。Flexレッグと予約制定時便の両方が参照する。予約ルールを持つフィードが無ければnull
...
interface ShardMeta {
  ...
  schemaVersion: 2;                 // 本章で規定するスキーマのバージョン。破壊的変更時にインクリメントし、packages/routerは未知バージョンを検出したらロードを拒否する
...
interface CompressedStopTimes {
  ...
  dropOffType: number[];
  pickupBookingRuleIdx?: (number | null)[];  // 乗車側の予約ルール参照(Shard.bookingRules配列内インデックス)。参照なしはnull。予約が必要な便を含まないシャードでは列自体を省略できる
  dropOffBookingRuleIdx?: (number | null)[]; // 降車側の予約ルール参照。仕様はpickupBookingRuleIdxと同じ
}
```

**予約ルール参照列を任意（optional）にする理由**: `pickup_type`/`drop_off_type`が2または3の「予約が必要な便」は全国664フィードのうち**27フィード**にしか存在しない（`20_企画確定.md` 1.2節）。大半のシャードでは全行が参照なしになるため、列を必須にすると無意味な`null`列を全シャードが背負う。省略時は「全行が参照なし」として扱う（`packages/router`の`loadShard()`が`?? -1`で正規化する）。

**予約ルールを`FlexData`の外に置く理由（schemaVersion 2、`17_課題管理.md` C-24）**: 高知県の6フィード（香南市・南国市・四万十市・四万十町・土佐清水市・高知県航空利用促進協議会）は`location_groups`も時間窓も持たず、**時刻表に載っている定時便に`pickup_type=2`と`pickup_booking_rule_id`が付いた形**である。この形は4.4節のFlexデータではなく通常の`stopTimes`として載るため、予約ルールが`FlexData`の内側にあると`flex: null`のシャードで予約ルールごと落ちる。予約ルールの正本をトップレベルに1つ置き、Flexレッグ（4.4節）と予約制定時便（本節）の両方が同じテーブルを参照する。
...
本節が扱うのは**エリア型デマンド交通**（`location_group`＋時間窓で任意地点を乗降する方式、瑞穂町「チョイソコみずほまち」が該当）である。「時刻表に載っているが乗るには電話予約が必要な定時便」（高知6フィードが該当）は本節ではなく4.3節の`stopTimes`＋`pickupBookingRuleIdx`で表現する。

```typescript
interface FlexData {
  locationGroups: CompressedLocationGroups;
  flexTrips: CompressedFlexTrips;
  // 予約ルールはShardのトップレベル（4.2節のbookingRules）が正本。schemaVersion 2でここから外した
}
...
// Shard.bookingRules（4.2節）の型。Flexレッグと予約制定時便（4.3節）が共通で参照する
...
|---|---|---|---|
| 1 | 2026-07-02 | 初版 | — |
| 2 | 2026-08-28 | `bookingRules`を`FlexData`からトップレベルへ移動。`CompressedStopTimes`に`pickupBookingRuleIdx`/`dropOffBookingRuleIdx`（任意列）を追加 | `17_課題管理.md` C-24。予約制の定時便に締切・連絡先を添付するため（`20_企画確定.md` §5.1） |
